...
async function fetchAndProcessAllDistricts() {
    ...
    addMessageToChatbox("Hello! The risk levels are now categorized into Red, Yellow, and Green zones.", 'ai');
}
...
// UPDATED: This function now uses the new "Zone" terminology
...
function updateCurrentConditions(current) { temperatureEl.textContent = `${current.main.temp.toFixed(1)} °C`; humidityEl.textContent = `${current.main.humidity} %`; rainfallEl.textContent = `${(current.rain?.['1h'] || 0).toFixed(1)} mm`; pressureEl.textContent = `${current.main.pressure} hPa`; windSpeedEl.textContent = `${(current.wind.speed * 3.6).toFixed(1)} km/h`; }
...
// UPDATED: This function now uses the new "Zone" terminology in the legend
...
// UPDATED: This function now uses the new "Zone" terminology in its responses
...
function extractMetrics(query) { const metrics = new Set(); const synonyms = { temperature: ['temperature', 'temp', 'hot', 'cold', 'degrees'], humidity: ['humidity', 'humid', 'moisture'], rain: ['rain', 'rainfall', 'precipitation'], risk: ['risk', 'danger', 'warning', 'red zone', 'yellow zone', 'green zone'], pressure: ['pressure'], wind: ['wind', 'windy'], general: ['weather', 'status', 'conditions', 'information', 'forecast'] }; for (const metric in synonyms) { if (synonyms[metric].some(word => query.includes(word))) { if (metric === 'general') { metrics.add('temperature').add('humidity').add('rain').add('risk').add('pressure').add('wind'); } else { metrics.add(metric); } } } return Array.from(metrics); }
function handleForecast(districtData, dateInfo, metrics) { const isRange = dateInfo.type === 'range'; const numDays = isRange ? dateInfo.value : 1; if (numDays <= 0 || numDays > 5) return { response: "Please ask for a forecast between 1 and 5 days.", data: {} }; const dailyData = {}; for (const item of districtData.forecastData.list) { const dateKey = new Date(item.dt * 1000).toISOString().split('T')[0]; if (!dailyData[dateKey]) dailyData[dateKey] = { temps: [], rains: [], dateObj: new Date(item.dt * 1000) }; dailyData[dateKey].temps.push(item.main.temp); dailyData[dateKey].rains.push(item.rain?.['3h'] || 0); } let relevantDays = Object.values(dailyData); if (isRange) relevantDays = relevantDays.slice(0, numDays); else relevantDays = relevantDays.filter(day => day.dateObj.getDate() === dateInfo.value.getDate()); if (relevantDays.length === 0) return { response: `I don't have a forecast for <b>${districtData.name}</b> on that date.`, data: {} }; let response = `Here is the forecast for <b>${districtData.name}</b>:`; const responseData = {}; for (const day of relevantDays) { const avgTemp = day.temps.reduce((a, b) => a + b, 0) / day.temps.length; const totalRain = day.rains.reduce((a, b) => a + b, 0); const dateString = day.dateObj.toLocaleDateString('en-IN', { weekday: 'long', day: 'numeric', month: 'short' }); response += `<br><br><b>${dateString}</b>:`; if (metrics.includes('temperature') || metrics.length === 0) { response += `<br>- Avg. Temp: <b>${avgTemp.toFixed(1)}°C</b>`; responseData.temperature = avgTemp; } if (metrics.includes('rain') || metrics.length === 0) { response += `<br>- Total Rain: <b>${totalRain.toFixed(2)} mm</b>`; responseData.rain = totalRain; } } return { response, data: responseData }; }
...
async function handleCurrentWeather(districtData, metrics) {
    ...
    if (!current) return { response: `Could not fetch live data for <b>${districtData.name}</b>.`, data: {} };
